...
// ---- Templates ----
function getTemplates() { return JSON.parse(localStorage.getItem('templates') || '[]'); }
function setTemplates(list) { localStorage.setItem('templates', JSON.stringify(list)); }
...
function renderTemplates() {
  ...
  el.innerHTML = list.map((t, i) => \`
    <div class="chip" data-idx="\${i}">
      <span class="load">\${escapeHtml(t.name)}</span>
      <span class="x" data-del="\${i}">✕</span>
    </div>
  \`).join('');
  el.querySelectorAll('.chip .load').forEach((span, i) => {
    span.addEventListener('click', () => {
      const t = list[i];
      $('f-name').value = t.nameText || '';
      $('f-tag').value = t.tag || '';
      $('f-caption').value = t.caption || '';
      $('f-btn-text').value = t.btnText || '';
      $('f-btn-url').value = t.btnUrl || '';
      updatePreview();
    });
  });
  el.querySelectorAll('.x').forEach(x => {
    x.addEventListener('click', (e) => {
      e.stopPropagation();
      const idx = parseInt(x.dataset.del, 10);
...
$('btn-save-tpl').addEventListener('click', () => {
  ...
  list.push({
    name,
    nameText: $('f-name').value,
    tag: $('f-tag').value,
    caption: $('f-caption').value,
    btnText: $('f-btn-text').value,
    btnUrl: $('f-btn-url').value
  });
  setTemplates(list);
  $('tpl-name').value = '';
  renderTemplates();
...
$('btn-post').addEventListener('click', async () => {
  ...
  const photoFile = $('f-image-file').files[0];
  const btnText = $('f-btn-text').value.trim();
  const btnUrl = $('f-btn-url').value.trim();
  const chatId = $('f-channel').value;
  const statusEl = $('status');

  if (!name) { statusEl.textContent = 'Name is required.'; statusEl.className = 'status err'; return; }
  if (!chatId) { statusEl.textContent = 'No channel configured on the server.'; statusEl.className = 'status err'; return; }
  ...
  form.append('chat_id', chatId);
  form.append('btnText', btnText);
  form.append('btnUrl', btnUrl);
  if (photoFile) form.append('photo', photoFile);

  try {
    const resp = await fetch('/api/post', { method: 'POST', body: form });
    const data = await resp.json();
    if (data.ok) {
      statusEl.textContent = 'Posted ✓';
      statusEl.className = 'status ok';
      saveToHistory(name, tag, caption, imageUrl || (photoFile ? '[uploaded image]' : ''));
      $('f-name').value = ''; $('f-tag').value = ''; $('f-caption').value = '';
      $('f-image-url').value = ''; $('f-image-file').value = '';
      $('f-btn-text').value = ''; $('f-btn-url').value = '';
      updatePreview();
    } else {
      statusEl.textContent = 'Error: ' + (data.description || 'unknown');
...
// ---- History ----
function saveToHistory(name, tag, caption, image) {
  const entry = { name, tag, caption, image: image || '', date: new Date().toISOString() };
  const list = JSON.parse(localStorage.getItem('posts') || '[]');
  list.push(entry);
  localStorage.setItem('posts', JSON.stringify(list));
}

function deleteHistoryEntry(realIdx) {
  const list = JSON.parse(localStorage.getItem('posts') || '[]');
  list.splice(realIdx, 1);
  localStorage.setItem('posts', JSON.stringify(list));
  loadHistory($('search-box').value);
}

function repostEntry(realIdx) {
  const list = JSON.parse(localStorage.getItem('posts') || '[]');
  const e = list[realIdx];
  if (!e) return;
  $('f-name').value = e.name || '';
  $('f-tag').value = e.tag || '';
...
function loadHistory(filter) {
  ...
  const withIdx = all.map((e, i) => ({ ...e, __idx: i })).reverse();
  const q = (filter || '').toLowerCase();
  const filtered = q ? withIdx.filter(e =>
    (e.name||'').toLowerCase().includes(q) || (e.tag||'').toLowerCase().includes(q) || (e.caption||'').toLowerCase().includes(q)
  ) : withIdx;

  if (filtered.length === 0) { listEl.innerHTML = '<div class="empty">No posts yet.</div>'; return; }

  listEl.innerHTML = filtered.map(e => \`
    <div class="post-item">
      <span class="name">\${escapeHtml(e.name)}</span>\${e.tag ? \`<span class="tag">#\${escapeHtml(e.tag)}</span>\` : ''}
      \${e.caption ? \`<div class="cap">\${escapeHtml(e.caption)}</div>\` : ''}
      <div class="date">\${new Date(e.date).toLocaleString()}</div>
      <div class="actions">
        <button class="btn-secondary" data-repost="\${e.__idx}">Repost</button>
        <button class="btn-secondary" data-delete="\${e.__idx}">Delete</button>
      </div>
    </div>
  \`).join('');

  listEl.querySelectorAll('[data-repost]').forEach(b => b.addEventListener('click', () => repostEntry(parseInt(b.dataset.repost, 10))));
  listEl.querySelectorAll('[data-delete]').forEach(b => b.addEventListener('click', () => deleteHistoryEntry(parseInt(b.dataset.delete, 10))));
}

$('search-box').addEventListener('input', (e) => loadHistory(e.target.value));
</script>
</body>
</html>`;
